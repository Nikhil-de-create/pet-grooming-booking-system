import { 
  ...
  businesses, users, services, staff, appointments, plans, subscriptions
} from "@shared/schema";
import { db } from "./db";
...
export class DatabaseStorage implements IStorage {
  ...
  // Method to initialize the database with default data
  async initializeDefaultData() {
    try {
      // Check if we already have any plans
      ...
      const [basicPlanRecord] = await db.insert(plans).values(basicPlan).returning();
      ...
      const [proPlanRecord] = await db.insert(plans).values(proPlan).returning();
      ...
      await db.insert(subscriptions).values(subscriptionData).returning();
      ...
      for (const staffData of defaultStaff) {
        await db.insert(staff).values(staffData).returning();
      }
      ...
      for (const service of defaultServices) {
        await db.insert(services).values(service).returning();
      }
